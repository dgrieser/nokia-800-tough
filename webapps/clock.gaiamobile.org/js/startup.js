...
define('sounds',['require','exports','module','l10n'],function(require, exports) {
  ...
  var SOUND_FILE_TO_L10N_ID = {
    ...
    'ac_alarm_1.mp3': 'ac_alarm_1_mp3',
    'ac_alarm_2.mp3': 'ac_alarm_2_mp3',
    'ac_alarm_3.mp3': 'ac_alarm_3_mp3',
    'ac_alarm_4.mp3': 'ac_alarm_4_mp3',
    'ac_alarm_5.mp3': 'ac_alarm_5_mp3',
...
define('navObjects',['require','alarm_database'],function(require) {
  ...
  var navObjects = {
    ...
    onAlarmChanged: function(event) {
      ...
      if (document.querySelector('.focus')) {
        lastEditAlarmId = document.querySelector('.focus').id;
      }
      switch (event.type) {
        case 'alarm-list-changed':
          if (navObj) {
            ...
            } else if(location.hash === '#alarm-edit-panel'){
                const elSounds = document.getElementById(lastEditAlarmId);
                const elEdit = document.querySelector('.focus');
                const selectedElement = elSounds || elEdit;

                if (selectedElement) {
                  NavigationManager.unfocus();
                  NavigationManager.setFocus(selectedElement);
              }
            }
          }
          break;
    ...
    /**
     * Responsible for normalize focus in input after onRing change.
     * For more details see bug 21424.
     * */
    onAlarmOnRingChanged: function(evt){
      if (location.hash === '#alarm-edit-panel') {
        const section = document.querySelector('section.alarm_edit');
        var hasInput = section.querySelector('.focus input[type=text]');
        if (hasInput) {
          //Use delay, confirm input can set focus again.
          setTimeout(() => { hasInput.focus(); }, 800);
        } else {
          section.querySelector('.focus').focus();
        }
      }
      evt.stopPropagation();
      evt.preventDefault();
  ...
  navObjects.goHome = function() {
    var cbGoHome = function() {
      setTimeout(function() {
        if (OptionHelper.softkeyPanel.menuVisible) {
          OptionHelper.softkeyPanel.hideMenu();
        }
        if (NavigationManager.navObjects.lastNavObject.name == 'alarm_settings') {
          location.hash = '#alarm-panel';
          //switch to alarm tab
          NavigationManager.unfocus();
          NavigationManager.navObjects.lastTab = "#alarm-panel";
    ...
    navObjects.appAction('GoHome', cbGoHome);
  };
  ...
  navObjects.init = function() {
    ...
    window.addEventListener('alarm-onring-changed', this.onAlarmOnRingChanged.bind(this));
...
var App = {
   ...
   * Use setTimeout lazy load page, avoid operation too fast.
   */
...
define('tmpl!panels/alarm/list_item.html',['tmpl'], function (tmpl) { return tmpl.toDom('<li class="alarm-cell" role="menuitem" tabindex="0">\n  <label class="alarmList pack-checkbox-large">\n    <input class="input-enable" data-id="" type="checkbox">\n    <span></span>\n  </label>\n  <a class="alarm-item" data-id="">\n    <div class="time p-pri"></div>\n    <div class="label h5"></div>\n    <div class="repeat p-sec"></div>\n  </a>\n</li>\n'); });
...
define('panels/alarm/alarm_list',['require','alarm_database','utils','l10n','app','tmpl!panels/alarm/list_item.html','async_queue','navObjects'],function(require) {
  ...
  toggleAlarm: function(alarm, enabled) {
    this.toggleAlarmQueue.push((done) => {
      if (enabled) {
        ...
        alarm.cancel(null, 'update').then(done);
      }
    });
  },
...
define('panels/alarm/active_alarm',['require','app','alarm_database','timer','utils','./child_window_manager','./post_message_proxy'],function(require) {
  ...
  var LOG_FOR_ALARM_DEBUGGING_ENABLED = false;
  ...
  if (LOG_FOR_ALARM_DEBUGGING_ENABLED) {
    // Log at startup.
  ...
  ActiveAlarm.prototype = {
    ...
    onTimerFired: function(data) {
      ...
      Timer.getFromStorage((err, timer) => {
        ...
        //===show second screen=========================
        var time = new Date(timer.startTime + timer.duration);
    ...
    close: function(type, alarmId) {
      ...
      if (actualHash === '#timer-panel' && type === 'timer') {
