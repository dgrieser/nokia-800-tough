...
define('panels/alarm_settings/main',['require','alarm','panels/alarm/clock_view','audio_manager','form_button','sounds','utils','l10n','panel','text!panels/alarm_settings/panel.html','constants'],function(require) {
  ...
  Utils.extend(AlarmSettings.prototype, {
    ...
    onFocusChanged: function(evt) {
        // Defect620 -huantan@t2mobile.com- The sound can't stop after settings.-begin
        var elVolume = document.querySelector('#settings-alarm-volume-control');
        if (!elVolume.classList.contains('focus') && this.ringtonePlayer._audio) {
            this.stopPreviewSound();
        }
        // Defect620 -huantan@t2mobile.com- The sound can't stop after settings.-end
      ...
      switch (evt.type) {
        case 'sound-save':
          ...
          if (lastTimerSound !== soundValue) {
            ...
            this.soundLabelSet(soundLabelData);
            this.soundRadioChecked(soundSelect);
          }
          this.backSettings();
          break;
      }
    ...
    soundRadioChecked: function(soundSelect) {
      var soundLastSelect = document.querySelector('.soundChecked');
    ...
    soundLabelSet: function(soundLabelData) {
