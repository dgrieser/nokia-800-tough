...
Timer.Panel = function(element) {
  ...
  this.nodes = {};
  this.tclPicker = this.element.querySelector('#kai-timer-time');
  ...
  NavigationManager.navObjects.items.timer.onChanged = function (argument) {
    var t1 = timeFromPicker(self.tclPicker.textContent);
    var t2 = 1000 * argument;
    var res = (t1 + t2) / 1000;
    if (res < 0 || res >= 24*60*60) { //range 0-23 Hours
      res = t1 / 1000;
    } else if (t2 == 0){
      self.tclPicker.dataset.time = NavigationManager.Utils.format.hms(0);
      self.tclPicker.classList.remove('focused');
      return;
    }
    self.tclPicker.dataset.time = NavigationManager.Utils.format.hms(res);
...
Timer.Panel.prototype.onclick = function(event) {
  ...
  if (panel.timer && panel.timer[meta.action]) {
    ...
    if (meta.action === 'create') {
      time = timeFromPicker(panel.tclPicker.textContent);
