...
(function(){var loader=LazyLoader;var isHoldingArrowUp=true;var isHoldingArrowDown=true;loader.load("js/navigation_map.js",function(){NavigationMap.init();});window.addEventListener("keydown",function(evt){handleKeydown(evt);if(evt.key==='ArrowUp'){isHoldingArrowUp=false;}else if(evt.key==='ArrowDown'){isHoldingArrowDown=false;}});window.addEventListener("keyup",function(evt){if(evt.key==='ArrowUp'){isHoldingArrowUp=true;}else if(evt.key==='ArrowDown'){isHoldingArrowDown=true;}});window.addEventListener("menuEvent",function(event){NavigationMap&&(NavigationMap.optionMenuVisible=event.detail.menuVisible);});function handleKeydown(evt){var el=evt.target,bestElementToFocus;if(typeof NavigationMap!='undefined'&&NavigationMap&&(NavigationMap.currentActivatedLength>0||NavigationMap.lockNavigation)){return;}
