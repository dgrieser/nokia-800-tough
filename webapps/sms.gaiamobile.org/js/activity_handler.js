...
let ActivityHandler = {
  ...
  _onActivity: function activityHandler(handlers, activity) {
    if (!MessageCacheRestore.isFTUDisabled) {
      ActivityHandler.showFTUinActivity();
      this.handlerBackup = handlers;
      this.activityBackup = activity;
    } else {
  ...
  */
  ...
  _onLatestMessageActivity: function() {
    let messagesComposeForm = document.getElementById('messages-compose-form');
    let messagesInput = document.getElementById('messages-input');
    messagesComposeForm.classList.add('hide');
    messagesInput.classList.remove('navigable');
    this._lastMessage = true;
    Navigation.toPanel('thread-list');
  },

  showFTUinActivity: function() {
    ThreadListUI.initElement();
    ThreadListUI.judgeNumberInputed();
    ThreadListUI.initSks();
  },

  nextFTUinActivity: function() {
    this._onActivityDirect(this.handlerBackup, this.activityBackup);
  },
  ...
  onSmsReceived: function ah_onSmsReceived(message) {
    // TODO It is not good idea, only a temporary method.
    // Need replace it by tag in the future.
    Utils.findBlockNumbers('number', message.sender).then((result) => {
      if (result.length === 0) {
      ...
      navigator.mozApps.getSelf().onsuccess = function(evt) {
        ...
        function continueWithNotification(sender, body) {
          ...
          if (Settings.hasSeveralSim() && message.iccId) {
            let simName = Settings.getSimNameByIccId(message.iccId);
            title = _(
          ...
          let options = {
            ...
            mozbehavior: {
              noscreen: true,
              soundFile: app.origin + ActivityHandler.notifyAudio
            }
          };
          ...
          // if the app is in background, we recover the focus
          // to resolve the background problem.
  ...
  // focus after visibility change, speed up notification launch.
  onSmsMessages: function() {
    function visibleTest() {
      if (!document.hidden) {
        document.removeEventListener('visibilitychange', visibleTest);
        let timeOut = 100;
        let container = document.getElementById('threads-container');
        if (!container.querySelector('.focus')) {
          let tofocus = container.querySelector('li');
    ...
    document.addEventListener('visibilitychange', visibleTest);
  },
