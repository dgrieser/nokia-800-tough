...
let Settings = {
  ...
  init: function settings_init() {
    ...
    /* << [CNT-1472]: BDC kanxj 20210812 Long SMS to be converted to MMS */
    Utils.getSettingsValue('ril.sms.defaultServiceId').then((value) => {
      if (value !== undefined) {
        Settings.smsServiceId = value;
        this.updateMaxConcatenatedMessages(value);
      }

      Utils.observerSettingsValue('ril.sms.defaultServiceId', (value) => {
        Settings.smsServiceId = value;
        this.updateMaxConcatenatedMessages(value);
      });
    });
 ...
 /* >> BTS-2381 */
  ...
  /* << BTS-2534: kanxj 20190812 WAP Push must be disabled for Vodafone AU (505-03) SIM cards */
  getwappushDefaultSettings: function getwappushDefaultSettings() {
    var slots = SIMSlotManager.getSlots();
    let isDualSim = Settings.isDualSimDevice();
    let matchInfo = {
        "clientId": "0",
 ...
 /* >> BTS-2534 */
  ...
  /* << [CNT-1472]: BDC kanxj 20210812 Long SMS to be converted to MMS */
  updateMaxConcatenatedMessages: function updateMaxConcatenatedMessages(value) {
    ...
    dump('updateMaxConcatenatedMessages ::value' + value);
    if (simSlot && simSlot.simCard && simSlot.simCard.iccInfo) {
      mccmnc = simSlot.simCard.iccInfo.mcc + "" + simSlot.simCard.iccInfo.mnc;
      dump('updateMaxConcatenatedMessages ::mccmnc' + mccmnc);
      if (mccmnc === '26006') {
        Settings.maxConcatenatedMessages = 6;
      } else {
        Settings.maxConcatenatedMessages = 10;
      }
    }
...
(function(exports) {
  ...
  let SettingsUI = {
    ...
    init: function sui_init() {
      ...
      if (!Startup.hasGroup) {
        let groupSettings =
            document.getElementById('group-message-number-set').parentNode;
    ...
    _openSelectItem: function() {
      ...
      if (focusedItems.length > 0) {
        ...
        if (viewPanel === 'group-number-set') {
          let dialogConfig = {
            title: 'device-phone-number',
            body: { 'text': Settings.defaultPhoneNumber },
            cancel: {
              l10nId: 'cancel',
              priority: 1
            },
            confirm: {
              ...
              callback: function (number) {
                let elem = document.getElementsByName('ril.mms.phoneNumber')[0];
                Utils.setSettingsValue({ 'ril.mms.phoneNumber': number });
                Settings.defaultPhoneNumber = number;
                if (number) {
                  elem.textContent = number;
                } else {
                  navigator.mozL10n.setAttributes(elem, 'no-phone-number');
                }
              }
            }
          };
        ...
        /* << BTS-2381: kanxj 20190731 add to support dualsim smsc config >>*/
          ...
          var slots = SIMSlotManager.getSlots();
          let isDualSim = Settings.isDualSimDevice();
          let smscNumberConfig = {
            title: 'Service center',
            body: { 'text' : focusItem.id === 'smsc-number-sim1' ? Settings.smscSim1 : Settings.smscSim2},
            cancel: {
              l10nId: 'cancel',
              priority: 1
            },
            confirm: {
              ...
              priority: 3,
              callback: function (number) {
                if (focusItem.id === 'smsc-number-sim1') {
          ...
          if (!slots[0].isAbsent() && (viewPanel === 'smsc-number-set-sim1')
                  && (Settings.smsc1EditEnable == 1)) {
            let dialog = new InputDialogHelper(smscNumberConfig);
            dialog.show(document.getElementById('file-large-confirmation-dialog'));
          } else if (isDualSim && !slots[1].isAbsent() && (viewPanel === 'smsc-number-set-sim2')
                  && (Settings.smsc2EditEnable == 1)){
            let dialog = new InputDialogHelper(smscNumberConfig);
            dialog.show(document.getElementById('file-large-confirmation-dialog'));
          }
        /* >> [BTS-415] */
    ...
    render: function() {
      ...
      ['ril.sms.requestStatusReport.enabled', 'ril.mms.requestStatusReport.enabled',
       'ril.mms.retrieval_mode', 'wap.push.enabled.sim1', 'wap.push.enabled.sim2',
       'ril.mms.phoneNumber', 'ril.sms.encoding_mode'].forEach((item) => {
        Utils.getSettingsValue(item).then((value) => {
          ...
          switch (item) {
            case 'ril.mms.retrieval_mode':
              ...
              if (value == '1') {
                elem.value = "manual";
              /* << BTS-2036: BDC kanxj 20190620 DUT unable to turn off "Auto retrieve messages" >> */
              } else if (value == '0') {
            ...
            case 'wap.push.enabled.sim2':
              ...
              break;
            case 'ril.mms.phoneNumber':
              Settings.defaultPhoneNumber = value;
              if (value) {
