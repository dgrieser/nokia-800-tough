  ...
  let VIEWS = {
    ...
    report: {
      ...
      handleKeyDown: function(e) {
        if ((e.key === 'Backspace') && !BlockContacts.atBlockPage) {
          e.preventDefault();
      ...
      updateContactList: function() {
        ...
        // There has black contact.
        if (ThreadUI.blockContactsList && ThreadUI.blockContactsList.length !== 0) {
          for (let i = 0; i < suggestionList.length; i++) {
            let nodeContact = suggestionList[i].getAttribute('data-number');
            for (let [index, item] of ThreadUI.blockContactsList.entries()) {
              let isBlock = /^\+/.test(item) ?
                item.indexOf(nodeContact) !== -1 :
                nodeContact.indexOf(item) !== -1;
              // Same number, add class, go through the next outer loop
              if (isBlock) {
                suggestionList[i].classList.add('suggestion-adjust');
                suggestionList[i].parentNode.classList.add('block-tag');
                break;
              }
              if (index === ThreadUI.blockContactsList.length - 1) {
          ...
          suggestionList.forEach(node => {
            node.classList.remove('suggestion-adjust');
            node.parentNode.classList.remove('block-tag');
          })
        }
      },
