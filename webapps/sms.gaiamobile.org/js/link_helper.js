...
(function() {
  ...
  */
  ...
  const leadingDigitsRegExp = /^[+0-9]+/;

  // ensure that each part of the domain is long enough
  function checkDomain(domain) {
    let ipv4RegExp = new RegExp('^(?:' + ipv4part + '\\.){3}' + ipv4part + '$');
  ...
  const safeStart = /[\s,:;\(>\u0080-\uFFFF]/;
  const notRadioCallChar = /^[+0-9]/;
  const regexpForLink = /[0-9]+\D+[0-9]+/;
  ...
  let LINK_TYPES = {
    phone: {
      ...
      matchFilter: function phoneMatchFilter(phone, link) {
        let onlyDigits = Utils.removeNonDialables(phone);
        ...
        let leadingDigits = leadingDigitsRegExp.exec(phone);
        if (leadingDigits &&
      ...
      transform: function phoneTransform(phone) {
        return '<a dir="ltr" data-dial="' + phone +
               '" data-action="dial-link" class="link-focus">' + phone + '</a>';
      }
    },
    ...
    url: {
      ...
      matchFilter: function urlMatchFilter(url, linkSpec) {
        ...
        scheme = match[1];
        tld = match[3] && match[3].slice(1).toLowerCase();
      ...
      transform: function urlTransform(url, linkSpec) {
        ...
        return '<a data-url="' + href +
               '" data-action="url-link" class="link-focus">' + url +
               '</a>';
      }
    },
    ...
    email: {
      ...
      transform: function emailTransform(email) {
        return [
          '<a data-email="',
          '" data-action="email-link" class="link-focus">',
          '</a>'
        ].join(email);
      }
    }
  ...
  let LINK_TYPES_KEYS = Object.keys(LINK_TYPES);

  function searchForLinks(type, string) {
    let linkSpecs = [];
    let spec = LINK_TYPES[type];
    ...
    let matchFilter = spec.matchFilter;
    let match, linkSpec;
    ...
    while (match = regexp.exec(string)) {
      ...
      let rest = string.slice(match.index - 1);
      if (match.index && !safeStart.test(rest.charAt(0))) {
        // we should only advance the regexp to the next safeStart
        let nextSafe = safeStart.exec(rest);
        if (nextSafe && (match[0].length > nextSafe.index)) {
          regexp.lastIndex = match.index + nextSafe.index;
          continue;
        }
      }

      // We need delete the not dial front char to make the phone more clear.
      if (type === 'phone' && !notRadioCallChar.test(match[0])) {
        if (!regexpForLink.test(match[0])) {
          match.index ++;
  ...
  function removeOverlapping(linkSpecs) {
    ...
    for (let index = 0; index < linkSpecs.length - 1; index++) {
      let end = linkSpecs[index].end;
  ...
  let LinkHelper = window.LinkHelper = {
    ...
    searchAndLinkClickableData:
     function lh_searchAndLinkClickableData(inputText, mode) {
      ...
      if (mode.hasOwnProperty('email') && !Settings.emailAppInstalled) {
        delete mode.email;
      }
      ...
      linkSpecs.forEach(function replaceLink(link) {
        ...
        let replacing = inputText.slice(link.start, link.end);
        result += LINK_TYPES[link.type].transform(replacing, link);
        lastEnd = link.end;
      });
