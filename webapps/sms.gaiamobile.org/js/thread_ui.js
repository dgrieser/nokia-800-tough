...
(function(exports) {
  ...
  let skBlockContacts = {
    ...
    priority: 3,
    method: function() {
      ThreadUI.blockContactList();
      ReportView.removeConfirmClose();
    }
  };
  ...
  let skChangeToindividual = {
    l10nId: 'individual-message',
    priority: 5,
    method: function () {
  ...
  let ThreadUI = {
    ...
    setGroupMessage: function thui_setGroupMessage(id) {
      let isGroup = Threads.get(id).isGroup;
      if (isGroup) {
        Settings.isGroup = true;
        Compose.updateType();
      }
    ...
    _showMessageSaveOrDiscardPrompt: function(close) {
      ...
      /* << [BTS-1609] BDC kanxj 201900522 add for Options should display 'select' when cancel exit */
      if (ThreadUI.contactPromptOptionMenuShown && close) {
      ...
      return new Promise((resolve, reject) => {
        ...
        function acceptCallback() {
          ThreadUI.saveDraft({ keyFlag: 'endCall' }, () => {
            resolve();
          });
        }
    ...
    handleMessageClick: function thui_handleMessageClick(evt) {
      ...
      if (elems.message.classList.contains('not-downloaded')) {
        ...
        Utils.exceedLimitSpace().then((exceedLimit) => {
          if (exceedLimit) {
    ...
    handleKeyEvent: function thui_handleKeyEvent(event) {
      ...
      if (Navigation.isCurrentPanel('thread')) {
        switch (event.key) {
          case 'Left':
          case 'ArrowLeft':
            if (!this.isInEditMode() && !this.attachInMessageInput() &&
        ...
        switch (event.key) {
          ...
          case 'EndCall':
            ...
            event.preventDefault();
            setTimeout(() => {
              exports.option.menuVisible = false;
            }, 0);
            break;
        }
      }
    ...
    attachInMessageInput: function() {
      return ((document.activeElement.parentNode.id === 'messages-input') &&
    ...
    updateInformationSks: function(callback) {
      /* There are two situation about block.
      *  1. Have contact recipient -> softkey is Cancel&Block.
      *  2. Not have contact recipient -> softkey is Cancel only.
      */
      this.pickContactsDestail().then((contacts) => {
        this.availableContact = contacts[0];
        this.blockContactsList = contacts[1];
        if (contacts[0].length === 0) {
          this.updateSKs(threadUiCancelReport);
        } else {
    ...
    pickContactsDestail: function() {
      return new Promise((resolve) => {
        let recipients = Threads.active.participants;
        let contact = [];
        let blockContact = [];
        let asyncTask = [];
        for (let i = 0; i < recipients.length; i++) {
          let newTask = () => new Promise((resolver) => {
            Contacts.findByAddress(recipients[i], (result) => {
              let detail;
              if (result.length !== 0) {
                let addresses = [];
                ...
                let detailList = Utils.getContactDetails(recipients[i], result, null);
                let dataList = null;

                addresses.forEach((current) => {
                  if (Utils.probablyMatches(current.value, recipients[i])) {
              ...
              Utils.findBlockNumbers('number', recipients[i]).then((result) => {
                if (result.length !== 0) {
    ...
    observeFocusChange_new: function() {
      ...
      let suggestionsObserver = new MutationObserver((mutations) => {
    ...
    scrollLinkElement: function (node, direction) {
      ...
      let secondNode = sourceElement
        .getElementsByClassName('message-content-body')[0];
      let secondElemnet = secondNode.querySelectorAll('.link-focus');
      let elementLength = secondElemnet.length;
      let nextFocusNode;
    ...
    blockContactList: function() {
      let informationPage = document.getElementById('information-report');
      let blockPage = document.getElementById('block-contact-page');

      informationPage.classList.add('hide');
      blockPage.classList.remove('hide');
      BlockContacts.init();
    },
    ...
    showMessageError: function thui_showMessageOnError(errorCode, opts) {
      let dialog = new ErrorDialog(Errors.get(errorCode), opts);
      dialog.show();
    },
    ...
    retrieveMMS: function thui_retrieveMMS(messageDOM) {
      ...
      if (!Settings.allowMmsWhenDataOff && !Settings.dataEnable) {
        ...
        if (message.type === 'mms') {
          ...
          Utils.exceedLimitSpace().then((exceedLimit) => {
            if (exceedLimit) {
              ...
              this.realResendMessage(id, message);
            }
          });
        } else {
          this.realResendMessage(id, message);
    ...
    realResendMessage: function(id, message) {
      ...
      if (message.type === 'sms' && !this.isSMSAbleToSend(serviceId)) {
        return;
      } else if (!this.isMMSAbleToSend(serviceId)) {
        return;
      }
      // Strategy:
      // - Delete from the DOM
    ...
    saveDraft: function(opts, callback) {
      ...
      Utils.exceedLimitSpace().then((exceed) => {
        if (callback) {
          if (!opts || (opts && opts.keyFlag !== 'endCall')) {
            callback();
          }
        }
        if (exceed) {
    ...
    showLowSpaceAlert: function() {
      const TOASTERTIMEOUT = 2000;
      Toaster.showToast({
        messageL10nId: 'space-low-not-save',
        latency: TOASTERTIMEOUT
      });
    },
    ...
    getSksForSaveDraft: function(skParam) {
      ...
      if (this.headerText.innerHTML === 'New Message (Individual)') {
        updateSkParam.items.push(skChangeToGroup);
      }
      if (this.headerText.innerHTML === 'New Message (Group)') {
        updateSkParam.items.push(skChangeToindividual);
      }
    ...
    dynamicComposerSK: function(focusState) {
      ...
      switch (focusState) {
        ...
        case this.FOCUS_ON_SUBJECT:
          ...
          if (this.recipients.length || !Compose.isEmpty()) {
    ...
    isAirplaneState: function() {
      ...
      if (conn && conn.radioState === 'disabled') {
         // If VoWofi connected, need send sms/mms when airplane mode on.
         if (imsHandler && (imsHandler.capability === 'voice-over-wifi' ||
    ...
    isMMSAbleToSend: function(serviceId) {
      ...
      if (!Settings.allowMmsWhenDataOff && !Settings.dataEnable) {
        if (conn && conn.radioState === 'disabled') {
          // If VoWofi connected
          console.log(" data disable , allowMmsWhenDataOff is false, and in airplane mode ");
          if (imsHandler && (imsHandler.capability === 'voice-over-wifi' ||
             imsHandler.capability === 'video-over-wifi')) {
            return true;
          }
    ...
    // See the free space before start send message.
    preSendMessage: function () {
      if (Compose.type === 'mms') {
        // we need reject send/download MMS if data is OFF,
        // please check bug 56531 for detail.
        /* BDC kanxj 20190627 add for DUT cannot send MMS over VoWiFi in flight mode start */
    ...
    startSendMessage: function () {
      ...
      if (serviceId === -1) {
        ...
        // The no SIM card situation for single SIM device should be considered.
        if (Settings._serviceIds.length === 0 &&
            !navigator.mozMobileConnections[serviceId].iccId) {
          this.showMessageError('NoSimCardError');
          return;
        }
